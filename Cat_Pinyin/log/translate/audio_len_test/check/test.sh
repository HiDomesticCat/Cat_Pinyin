	...
	do
		...
		do
			...
			do
				...
				check=0 #記錄檔案是否為空音檔
				`cmp --silent "word_""$loop""_""$loop2""_""$loop3""_""$loop4"".ts" word_1_3_13_4.ts || echo "files are different"` #檢測檔案是否與空音檔相同
				#if [ $check -eq 1 ]; then echo `rm "word_""$loop""_""$loop2""_""$loop3""_""$loop4"".ts"`; #是就刪除
